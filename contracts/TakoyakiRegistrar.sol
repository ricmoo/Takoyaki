...
contract TakoyakiRegistrar {
    ...
    event Registered(address indexed owner, uint256 tokenId, uint48 expires);
    ...
    Resolver private _defaultResolver = Resolver(0x5FfC014343cd971B7eb70732021E26C35B744cc4);
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        string memory uri = "https://meta.takoyaki.cafe/json/_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-";

        // Offset into the URI to replace. Skips
        //  - the length prefix (32 bytes)
        //  - the "https://meta.takoyaki.cafe/json/" (32 bytes)
        uint offset = 0;
    ...
    function reveal(string calldata label, address owner, bytes32 salt) external {
        ...
        Commitment memory commitment = _commitments[blindedCommit];
        require(commitment.blockNumber <= block.number + MAX_COMMIT_BLOCKS);
        require(commitment.blockNumber >= block.number + MIN_COMMIT_BLOCKS);
        ...
        emit Registered(owner, tokenId, takoyaki.expires);
    ...
    function renew(uint256 tokenId) external payable {
        ...
        emit Registered(takoyaki.owner, tokenId, takoyaki.expires);
    }
    ...
    function available(uint256 tokenId) external view returns (bool) {
        return (_takoyaki[tokenId].expires < now - GRACE_PERIOD);
    }

    function expires(uint256 tokenId) external view returns (uint48) {
        return _takoyaki[tokenId].expires;
    }
