...
function submitReveal(signedTx, local) {
    ...
    var url = "https://takoyaki.cafe/reveal/";
    if (local) {
        url = "http://takoyaki.local:5000/reveal/";
    }
    return ethers_1.utils.fetchJson(url + reveal.substring(2) + "/").then(function (result) {
                    ...
                    case 1:
                        ...
                        if (genes.name) {
                            ...
                            promises.name = this.queryFilter(filter, genes.revealBlock, genes.revealBlock).then(function (events) {
                                ...
                                if (result) {
                                    promises.addr = _this.provider.resolveName(name + ".takoyaki.eth");
                                }
                                return result;
                            });
                        }
