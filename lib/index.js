...
function _getTraits(genes, hashCache) {
    let state = 5;
    if (!genes.commitBlock || !hashCache.get(genes.commitBlock + 1)) {
        state = 0;
    } else if (!hashCache.get(genes.commitBlock + 3)) {
       state = 1;
    } else if (!genes.revealBlock || !hashCache.get(genes.revealBlock + 1)) {
       state = 2;
    } else if (!hashCache.get(genes.revealBlock + 3)) {
       state = 3;
    } else if (!hashCache.get(genes.revealBlock + 5)) {
       state = 4;
    }
    ...
    let tattoo = mouth.subRandom(hashCache.get(genes.revealBlock + 3) || genes.salt);
