...
function _getTraits(genes, hashCache) {
    let state = 5;
    if (genes.commitBlock === 0 || !hashCache.get(genes.commitBlock + 1)) {
        state = 0;
       ...
       state = 3;
    } else if (!hashCache.get(genes.revealBlock + 3)) {
       state = 4;
    }
...
const contractAddress = "0x93285db68799256f562b446e0b420104f9e65721";

const ABI = [
    "function getTakoyakiTraits(uint256 tokenId) view returns (bytes32 salt, uint48 commitBlock, uint48 revealBlock, uint48 expires)",
    ...
    "function reveal(string label, address owner, bytes32 salt) @200000",
...
TakoyakiContract.prototype.getTraits = function(tokenId, hints) {
    ...
    return Promise.all([
        ...
        this.functions.getTakoyakiTraits(tokenId)
    ]).then((results) => {
        let blockNumber = results[0];
        let traits = results[1];
        console.log("getTakoyakiTraits:", traits);
        ...
        if (genes.commitBlock === 0) { return _getTraits(genes, HashCache); }

        let addBlock = (blockNumber) => {
            console.log("ADD", blockNumber);
            return this.provider.getBlock(blockNumber).then((block) => {
        ...
        if (!HashCache.get(genes.revealBlock + 1)) {
            promises.push(addBlock(genes.revealBlock + 1));
        }

        if (!(HashCache.get(genes.revealBlock + 2) && HashCache.get(genes.revealBlock + 3))) {
...
TakoyakiContract.prototype.commit = function(label, owner, salt, prefundRevealer, prefundAmount) {
    ...
    console.log("blinded1", overrides, label, owner, salt, prefundRevealer, prefundAmount);
    return this.functions.makeBlindedCommitment(label, owner, salt).then((blindedCommit) => {
    console.log("blinded2", blindedCommit);
...
module.exports = {
    ...
    labelToUrl: function(label, local) {
       console.log(label, local, toASCII(label));
        if (local) {
            return "http://" + toASCII(label) + ".takoyaki.local:8000/";
        }
        return "https://" + toASCII(label) + ".takoyaki.cafe";
    },

    connect: function(signerOrProvider) {
        return new TakoyakiContract(contractAddress, ABI, signerOrProvider);
    },
